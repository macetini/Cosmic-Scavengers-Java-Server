...
/**
 * Utility for handling final network message framing and sending using Netty's
 * ByteBuf. This ensures the message conforms to the custom protocol: [4-byte
 * Length (Big-Endian)] + [1-byte Type] + [Payload]
 */
public class NettyResponseFramer {
	private NettyResponseFramer() {
		// Utility class
	}
	...
	/**
	 * Takes the serialized payload and wraps it in the final network frame, then
	 * sends it. * @param ctx The Netty ChannelHandlerContext.
	 * 
	...
	public static void sendFramedResponse(ChannelHandlerContext ctx, byte[] payload, byte messageType) {
		...
		final int payloadLength = 1 + payload.length;

		// Total frame length = 4 (Length prefix) + Payload length
		final int totalFrameLength = 4 + payloadLength;

		// 1. Allocate buffer using the context's allocator (recommended Netty practice)
		// We set the ByteOrder to BIG_ENDIAN for the Length Prefix, as the client
		// reverses it for compatibility.
		final ByteBuf buffer = ctx.alloc().buffer(totalFrameLength).order(ByteOrder.BIG_ENDIAN);

		// --- 1. Write 4-byte Length Prefix (Big-Endian) ---
		buffer.writeInt(payloadLength);

		// --- 2. Write 1-byte Message Type ---
		// Since this is a single byte, endianness doesn't matter, but it is written
		// after the length.
		...
		if (log.isWarnEnabled()) { // Use isWarnEnabled to avoid string formatting overhead if debug is off
	...
	/**
	 * Alternative method to convert a raw byte array (produced by a handler) into a
	 * ByteBuf for sending, if the handler already includes the framing.
	 */
	public static void sendRawBuffer(ChannelHandlerContext ctx, byte[] framedBuffer) {
		final ByteBuf buffer = ctx.alloc().buffer(framedBuffer.length);
